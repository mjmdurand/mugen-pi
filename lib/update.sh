...
if [ ! -f ${DESKTOP_PATH}/karaokeMugenUpdate.desktop ];then
echo '[Desktop Entry]
Name=Karaoke Mugen Update
Comment=Update Karaoke Mugen
Icon='${MUGEN_PI_DIR}'/img/updateicon.png
Exec=gnome-terminal -t "Update Karaoke Mugen" --working-directory='${MUGEN_PI_DIR}' -e ./update.sh
Type=Application
Encoding=UTF-8
Terminal=false
Categories=None;' >> ${DESKTOP_PATH}/karaokeMugenUpdate.desktop
chmod u+x ${DESKTOP_PATH}/karaokeMugenUpdate.desktop
